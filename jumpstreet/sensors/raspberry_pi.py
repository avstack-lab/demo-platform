...
class RaspberryPi(Sensor):
    ...
    def __init__(self, context, identifier, configs, host="127.0.0.1", backend=DEFAULT_BACKEND_PORT, 
        ...
        super().__init__(context=context, identifier=identifier, type="camera-raspberry-pi", configs=configs, 
    ...
    def start_capture(self):
        ...
        t0 = 0
        frame_counter = 0
        for frame in self.handle.capture_continuous(stream, format='jpeg', use_video_port=True):
            ...
            img_raw = cv2.imdecode(img_raw, cv2.IMREAD_COLOR)  # np.ndarray with d = (h, w, 3)
